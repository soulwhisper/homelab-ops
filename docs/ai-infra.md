...
flowchart TB
    ...
    subgraph LLM["LLM Providers"]
        ...
        SF-Vision["Qwen3-VL-30B-A3B<br/>(external API)"]
        SF-Claude["GLM-5.2<br/>(Anthropic-compat API)"]
    end
...
| `x-model: vision` | `llm-backend-vision` | `Qwen/Qwen3-VL-30B-A3B-Instruct` | External API |
| `x-model: claude-*` (regex) | `llm-backend-anthropic` | `zai-org/GLM-5.2` | External API (Anthropic compat) |
...
x-model: vision   ──────────► Qwen3-VL-30B-A3B       (external, paid API)
x-model: claude-* ──────────► GLM-5.2                (external, paid API)
```
